...
Original file is located at
    https://colab.research.google.com/drive/1i4j4_E2CsF88Ix2eltLob3xCjBH8X820
"""
...
def prepare_image(path,factor):
    ...
    for file in os.listdir(path):
        ...
        # cuvamo modifikovane slike u direktorijumu slike1
        cv2.imwrite('drive/MyDrive/slike1/{}'.format(file),img)

# testiramo slike koriscenjem metrika za kvalilet slike
# PSNR,MSE,SSIM
def test():
    for file in os.listdir('drive/MyDrive/slike1/'):
      imageA = cv2.imread('drive/MyDrive/slike1/{}'.format(file))
      imageB = cv2.imread('drive/MyDrive/slike/Set14/{}'.format(file))
      Psnr = psnr(imageA,imageB)
      Mse = mean_squared_error(imageA,imageB)
      SSIM = ssim(imageA,imageB,multichannel=True)
      print('file:{}\nPSNR:{}\nMSE:{}\nSSIM:{}\n'.format(file,Psnr,Mse,SSIM))
...
def main():
    prepare_image("drive/MyDrive/slike/Set14",2)    
    test()
...
def model():  
  SRCNN = Sequential([
    Conv2D(filters=128,kernel_size=(9,9),padding='valid',activation='relu',kernel_initializer = 'glorot_uniform',use_bias=True,input_shape = (None,None,1)),
    Conv2D(filters=64,kernel_size=(3,3),padding='same',activation='relu',kernel_initializer = 'glorot_uniform',use_bias=True),
    Conv2D(filters=1,kernel_size=(5,5),padding='valid',activation='linear',kernel_initializer = 'glorot_uniform',use_bias=True)
  ],name = 'MyModel')
...
def predict(image_path):
  ...
  ref = cv2.imread('drive/MyDrive/slike/Set14/{}'.format(file_name))  
  ...
  # temp.shape[0] - sirina<--
  # temp.shape[1] - visina<-|
  # slike koja je dobijena od slike degraded promenom prostora boja u YCbCr
...
ref,degraded,output,scores = predict('drive/MyDrive/slike1/flowers.bmp')
print('Rezultati za sliku flowers.bmp')
...
for image in os.listdir(path_to_dir):
  ref,degraded,output,scores = predict('drive/MyDrive/slike1/{}'.format(image))
  images = [ref,degraded,output]
  f,axs = plt.subplots(nrows=1,ncols=3,figsize = (20,8))
  
  
  axs[0].imshow(images[0])
  axs[0].set_title(types[0],fontdict={'fontsize': 25, 'fontweight': 'medium','color':'blue'})
  axs[0].set_xticks([])
  axs[0].set_yticks([])
  ...
  for j in range(1,3):
    ...
    axs[j].set_title(types[j-1],fontdict={'fontsize': 25, 'fontweight': 'medium','color':'blue'})
    axs[j].set_xticks([])
    axs[j].set_yticks([])